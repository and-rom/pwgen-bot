...
$START .= "Я Telegram бот, меня зовут *FlimFlamBot*. Я могу придумывать сложные и легко запоминающиеся пароли на основе парольных фраз.";
...
$HELP .= "Подробнее о паролях: /help\\_about". PHP_EOL;
$HELP .= "Подробнее о команде /pw: /help\\_pw";

const HELP_ABOUT = <<<EOD
Бот предназначен для создания случайных паролей любого уровня сложности, которые формируются на основе легко запоминающихся парольных фраз. 
...
Бот может сформировать пароль длиной от 6 до 24 символов, включающих цифры и буквы в верхнем и нижнем регистрах.

Пароль, образованный из парольной фразы, начинается с числа, если оно задано, затем включает по N первых букв от каждого из M слов парольной фразы без пробелов.
...
Фраза для запоминания - 10 Белых Птичек Кромсают Лгуна

Подробнее о команде /pw: /about\\_pw
EOD;

$HELP_PW = "Команда /pw может иметь аргумент, содрежащий параметры генерации пароля. Параметры задаются в виде числа после символа подчеркивания. Например, /pw\\_424" . PHP_EOL;
...
$HELP_PW .= "Например, если ввести только две цифры (/pw\\_31), то будут заданы параметры количества слов и количества цифр. ";
$HELP_PW .= "В параметры, значения которых не заданы, будут подставлены значения по умолчанию." . PHP_EOL;
$HELP_PW .= "В парольной фразе полужирным шрифтом выделены те символы, которые используются в пароле.";

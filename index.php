...
const START = <<<EOD
_Привет!_
Я Telegram бот, меня зовут *FlimFlamBot*. Я могу придумывать сложные и легко запоминающиеся пароли на основе парольных фраз.
...
const HELP_PW = <<<EOD
Команда /pw может иметь аргумент, содрежащий параметры генерации пароля. Параметры задаются в виде числа после символа подчеркивания. Например, /pw\\_424.

Цифр в числе может быть от 0 до 5.
- *Первая цифра* - количество слов во фразе (значения от 3 до 5, по умолчанию - 3).
- *Вторая цифра* - количество цифр в начале фразы (значения от 0 до 4, по умолчанию - 0).
- *Третья цифра* - количество букв из каждого слова (значения от 2 до 4, по умолчанию - 3).
...
В парольной фразе полужирным шрифтом выделены те символы, которые используются в пароле.
EOD
